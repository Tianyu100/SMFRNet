...
# from src.dla34_add_basnet import myDLA34 
# from dla34_add_basnet import myDLA34
...
def main(args):
    ...
    train_dataset = DUTSDataset(args.data_path, train=True, transforms=SODPresetTrain([256,256], crop_size=256))

    val_dataset = DUTSDataset(args.data_path, train=False, transforms=SODPresetEval([256,256]))
...
#     model = GeleNet() 
    ...
    model.to(device)
    model_name = "SeaNet"
    # 用来保存训练以及验证过程中信息，文件名中包含模型名
    results_file = "results{}_{}.txt".format(model_name, datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_interval == 0 or epoch == args.epochs - 1:
            ...
            mae_metric,f1_metric,sm = evaluate(model, val_data_loader, device=device)
            mae_info, f1_info = mae_metric, f1_metric

            print(f"[epoch: {epoch}] val_MAE: {mae_info:.3f} val_mF1: {f1_info:.3f} val_Sm: {sm:.3f}")
            # write into txt
            with open(results_file, "a") as f:
                # 记录每个epoch对应的train_loss、lr以及验证集各指标
                write_info = f"[epoch: {epoch}] train_loss: {mean_loss:.4f} lr: {lr:.6f}" \
                             f"MAE: {mae_info:.3f} maxF1: {f1_info:.3f} Sm: {sm:.3f}\n"
                f.write(write_info)
                
            ####Tensorboard########
#             writer.add_scalar('Loss/train_loss',mean_loss,epoch)
#             writer.add_scalar('Loss/val_loss',val_loss,epoch)
#             writer.add_scalars('val', {'MAE': mae_info, 'm_F1':f1_info,'Sm':sm}, epoch)
#             writer.close()
...
#         only save latest 10 epoch weights
        if os.path.exists(f"save_weights/model_{epoch-50}.pth"):
            os.remove(f"save_weights/model_{epoch-50}.pth")
...
def parse_args():
    ...
    parser.add_argument("--data-path", default="./zdata4_m", help="DUTS root")
    ...
    parser.add_argument("--eval-interval", default=4, type=int, help="validation interval default 10 Epochs")

    parser.add_argument('--lr', default=0.001, type=float, help='initial learning rate')
    parser.add_argument('--print-freq', default=100, type=int, help='print frequency')
    parser.add_argument('--resume', default='', help='resume from checkpoint')#save_weights/model_42.pth
    parser.add_argument('--start-epoch', default=0, type=int, metavar='N',
                        help='start epoch')
    # Mixed precision training parameters
    parser.add_argument("--amp",action='store_true',
                        help="Use torch.cuda.amp for mixed precision training")#default=False
